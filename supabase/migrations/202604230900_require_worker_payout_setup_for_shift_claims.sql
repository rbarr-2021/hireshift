...
begin
  ...
  select stripe_connect_details_submitted, stripe_connect_payouts_enabled
  into worker_payout_details_submitted, worker_payouts_enabled
  ...
  if coalesce(worker_payout_details_submitted, false) is not true
    or coalesce(worker_payouts_enabled, false) is not true then
    raise exception 'Set up Stripe payouts before taking your first shift';
  end if;
