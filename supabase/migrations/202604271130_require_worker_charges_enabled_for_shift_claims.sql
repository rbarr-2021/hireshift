...
declare
  ...
  worker_payout_details_submitted boolean;
  worker_payouts_enabled boolean;
  worker_charges_enabled boolean;
...
begin
  ...
  select stripe_connect_details_submitted, stripe_connect_payouts_enabled, stripe_connect_charges_enabled
  into worker_payout_details_submitted, worker_payouts_enabled, worker_charges_enabled
  ...
  if coalesce(worker_payout_details_submitted, false) is not true
    or coalesce(worker_payouts_enabled, false) is not true
    or coalesce(worker_charges_enabled, false) is not true then
    raise exception 'You need to finish your payout setup with Stripe before accepting paid shifts.';
  end if;
  ...
  if shift_start <= timezone('utc', now()) then
  ...
  shift_end := (
    coalesce(target_listing.shift_end_date, target_listing.shift_date)::text || ' ' || target_listing.end_time::text
  )::timestamp;

  if exists (
    select 1
    from public.bookings existing_booking
    where existing_booking.worker_id = current_user_id
      and existing_booking.status in ('pending', 'accepted')
      and (
        (existing_booking.shift_date::text || ' ' || existing_booking.start_time::text)::timestamp
      ) < shift_end
      and (
        (
          coalesce(existing_booking.shift_end_date, existing_booking.shift_date)::text
          || ' '
          || existing_booking.end_time::text
        )::timestamp
      ) > shift_start
  ) then
    raise exception 'You already have another accepted shift during this time';
  end if;
